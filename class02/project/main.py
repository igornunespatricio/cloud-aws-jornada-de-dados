...
def upload_arquivos_para_s3(arquivos: List[str]) -> None:
    ...
    for arquivo in arquivos:
        ...
        try:
            print(
                f"Tentando fazer upload de '{nome_arquivo}' para o bucket '{BUCKET_NAME}'..."
            )
...
def executar_backup(pasta: str) -> None:
    ...
    try:
        ...
        if arquivos:
            ...
            # deletar_arquivos_locais(arquivos) -- optional to delete files from local data folder
        else:
...
if __name__ == "__main__":
    PASTA_LOCAL: str = (
        "class02/project/data"  # Substitua pelo caminho da sua pasta local
    )
    try:
